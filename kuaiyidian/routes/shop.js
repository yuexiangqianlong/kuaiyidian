...
router.get('/', async function(ctx, next) {
	...
	if (typeof(loginbean.shoprole) != 'undefined' && loginbean.shoprole == 0) {
		...
		let rs = await ShopUserModel.findAll({
			where: {
				shopid: loginbean.shopid
			}
		});
		await ctx.render('shop/shopIndex', {
			rs: rs
		});
	} else {
		ctx.redirect('/');
...
router.post('/login', async function(ctx, next) {
	let rs = await ShopUserModel.findOne({
		where: {
...
router.post('/mobileLogin', async function(ctx, next) {
	...
	if (rs) {
		...
		let shopRs = await sequelize.query(sql, {
			replacements: [rs.shopid]
		});
		loginbean.id = rs.id;
		loginbean.nicheng = rs.nicheng;
		loginbean.shoprole = rs.role;
...
router.get('/menu', async function(ctx, next) {
	...
	if ((typeof(loginbean.shoprole) != 'undefined') && loginbean.shoprole == 0) {
		let rs = await MenuModel.findAll({
			where: {
				shopid: loginbean.shopid
			}
		});
		await ctx.render('shop/menu', {
			rs: rs
		});
	}
})
...
router.get('/dishes', async function(ctx, next) {
	...
	if (loginbean.shoprole == 0) {
		...
		let rs = await DishModel.findAll({
			where: {
				menuid: id,
				shopid: loginbean.shopid
			}
		});
		await ctx.render('shop/dishmanage', {
			dishRs: rs
		});
	} else {
		ctx.body = '你无权查看';
...
router.post('/addDish', async function(ctx, next) {
	...
	if (loginbean.shoprole == 0) {
		...
		try {
			...
			let updrs = await MenuModel.update({
...
router.post('/updDish', async function(ctx, next) {
	...
	if (loginbean.shoprole == 0) {
		...
		fields.shopid = loginbean.shopid;
		try {
			let updrs = await DishModel.update(fields, {
				where: {
					'id': fields.dishid
				}
			});
...
router.get('/delDish', async function(ctx, next) {
	...
	let delrs = await sequelize.query(sql, {
		replacements: [dishid, loginbean.shopid],
		type: sequelize.QueryTypes.DELETE
	});
	sql = 'update menus set num=num-1 where id=? and shopid=?';
	let updrs = await sequelize.query(sql, {
		replacements: [menuid, loginbean.shopid],
